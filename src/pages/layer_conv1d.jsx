...
const Layer_conv1d = () => {
  ...
  useEffect(() => {
    const fetchAllHomes = async () => {
      try {
        const res = await axios.get('https://backend-code-0wjy.onrender.com/conv1d');
        setHome(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    if (home.length > 0) {
      ...
      const keyword = '@keras_export("keras.layers.Conv1D", "keras.layers.Convolution1D")';
      const keyword2 = 'Convolution1D = Conv1D';
  ...
  return (
    <div className="codemirror-container">
      ...
      <div className="editor-container">
        <h1>layers.Conv1D || layers.Convolution1D</h1>
        <CodeMirror
          value={code}

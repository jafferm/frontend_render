...
const Layer_simpleRNN = () => {
  ...
  useEffect(() => {
    const fetchAllHomes = async () => {
      try {
        const res = await axios.get('https://backend-code-0wjy.onrender.com/simprnn');
        setHome(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    if (home.length > 0) {
      ...
      const keyword = '@keras_export("keras.layers.SimpleRNN")';
      const keyword2 = 'return cls(**config)';
...
export default Layer_simpleRNN;

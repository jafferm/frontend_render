...
const Layer_cudnn = () => {
  ...
  useEffect(() => {
    const fetchAllHomes = async () => {
      try {
        const res = await axios.get('https://backend-code-0wjy.onrender.com/cudnn_lstm');
        setHome(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    if (home.length > 0) {
      ...
      const keyword = '@keras_export(v1=["keras.layers.CuDNNLSTM"])';

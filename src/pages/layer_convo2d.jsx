...
const Layer_convo2d = () => {
  ...
  useEffect(() => {
    if (cont.length > 0) {
      ...
      const keyword = '#excel sheet row 84';
  ...
  useEffect(() => {
    const fetchAllHomes = async () => {
      try {
        const res = await axios.get('https://backend-code-0wjy.onrender.com/convo2d');
        setHome(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    if (home.length > 0) {
      // Specify the keyword or pattern to search for
      const keyword = '@keras_export("keras.layers.Conv2D", "keras.layers.Convolution2D")';
      const keyword2 = 'Convolution2D = Conv2D';
  ...
  return (
    <div className="codemirror-container">
      ...
      <div className="editor-container">
        <h1>layers.Conv2D || layers.Convolution2D</h1>

...
const Layer_repvec = () => {
  ...
  useEffect(() => {
    const fetchAllHomes = async () => {
      try {
        const res = await axios.get('https://backend-code-0wjy.onrender.com/repvec');
        setHome(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    if (home.length > 0) {
      ...
      const keyword = '@keras_export("keras.layers.RepeatVector")';
      const keyword2 = 'return dict(list(base_config.items()) + list(config.items()))';

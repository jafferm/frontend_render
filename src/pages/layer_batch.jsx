...
const Layer_batch = () => {
  ...
  useEffect(() => {
    const fetchAllHomes = async () => {
      try {
        const res = await axios.get('https://backend-code-0wjy.onrender.com/batch_normalization');
        setHome(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    if (home.length > 0) {
      ...
      const keyword = '@keras_export("keras.layers.BatchNormalization", v1=[])';
      const keyword2 = '@keras_export';

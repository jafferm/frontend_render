...
const Layer_maxpool = () => {
  ...
  useEffect(() => {
    const fetchAllHomes = async () => {
      try {
        const res = await axios.get('https://backend-code-0wjy.onrender.com/maxpool');
        setHome(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    if (home.length > 0) {
      ...
      const keyword = '@keras_export("keras.layers.MaxPooling2D", "keras.layers.MaxPool2D")';
      const keyword2 = 'MaxPool2D = MaxPooling2D';
  ...
  return (
    <div className="codemirror-container">
      ...
      <div className="editor-container">
        <h1>keras.layers.MaxPooling2D || keras.layers.MaxPool2D</h1>
        <CodeMirror
          value={code}

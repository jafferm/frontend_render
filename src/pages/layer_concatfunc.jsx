...
const Layer_concatfunc = () => {
  ...
  useEffect(() => {
    const fetchAllHomes = async () => {
      try {
        const res = await axios.get('https://backend-code-0wjy.onrender.com/concatenate');
        setHome(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    if (home.length > 0) {
      ...
      const keyword = 'def concatenate(inputs, axis=-1, **kwargs):';
      const keyword2 = 'return Concatenate(axis=axis, **kwargs)(inputs)';
      ...
      const selectedCode = home.reduce((accumulator, homeItem) => {
        ...
        const selectedLines = fileLines.slice(startIndex, endIndex);
...
export default Layer_concatfunc;
